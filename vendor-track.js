...
var brpop = function () {
    console.log('brpop')
    client.brpop("vendor:track", 5, function (err, response) {
        if (response) {
            var value = JSON.parse(response[1])
            // 秘钥
            if (value.key) {
                key = value.key
            }
            // 加密content
            var content = aesEncrypt(value.content, key)
            var buff = Buffer.from(JSON.stringify(content))
